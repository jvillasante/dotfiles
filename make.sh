...
main() {
    ...
    for file in $files; do
        [ -L "$HOME/.claude/$file" ] && unlink "$HOME/.claude/$file"
        ln -s "$DOTFILES_DIR/Common/claude/$file" "$HOME/.claude"
    done
    ...
    files="emacs.service xremap.service"
    for file in $files; do
    ...
    if [ -d "$HOME/Workspace/Work/Omicron/Projects" ]; then
        ...
        if [ -d "$HOME/Workspace/Work/Omicron/Projects/nntp/master" ]; then
            ...
            ln -s "$DOTFILES_DIR/Misc/work/nntp/.clang-tidy" "$HOME/Workspace/Work/Omicron/Projects/nntp/master/"

            [ -L "$HOME/Workspace/Work/Omicron/Projects/nntp/master/.dir-locals.el" ] &&
