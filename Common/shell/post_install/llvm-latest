...
sudo bash /tmp/llvm.sh "${version}" all
...
master="clang"
slaves="asan_symbolize bugpoint clang++ clang-cpp clangd clang-format clang-tidy count dsymutil FileCheck ld64.lld ld.lld llc lld lldb lldb-argdumper lldb-instr lldb-server lldb-vscode lld-link lli lli-child-target not obj2yaml opt sanstats split-file UnicodeNameMappingGenerator verify-uselis
