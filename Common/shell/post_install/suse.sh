...
usage() {
    ...
    echo
    echo " e.g: $SCRIPT_NAME zipper update"
    exit "$1"
}
        ...
        case $CHOICE in
            ...
            10)
                ...
                [ ! -f "$KEYS_DIR/gnupg.tar.gz.gpg" ] &&
                    notify-send "$KEYS_DIR/gnupg.tar.gz.gpg does not exists" --expire-time=20 && continue
                ...
                [ ! -d "$KEYS_DIR"/gnupg ] && notify-send "Decryption failed, $KEYS_DIR/gpg does not exists" \
                                                        --expire-time=20 && continue
