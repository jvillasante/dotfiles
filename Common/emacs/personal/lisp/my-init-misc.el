...
(use-package ediff
    ...
    (defvar my/ediff-revision-session-p nil
        "If t then `my/ediff-revision-actual' has been called.
This state variable is used to insert added behavior to the overridden
function `ediff-janitor'.")

    (defun my/ediff-revision-from-menu (e)
...
This function will also test if a diff should apply to the current buffer."
    ...
    (defun my/ediff-revision-actual ()
...
This function handles the actual diff behavior called by `ediff-revision'."
        (let ((rev1 "")
                 (rev2 ""))
            (setq my/ediff-revision-session-p t)
...
CC MODIFICATION: This method overrides the original Ediff function."
        (let ((ask (if (and (boundp 'my/ediff-revision-session-p)
                           my/ediff-revision-session-p)
                       nil
                       ask)))
            (ediff-dispose-of-variant-according-to-user
                ediff-buffer-A 'A ask keep-variants)
            ;; !!!: CC Note: Test global state variable `my/ediff-revision-session-p' to
            ...
            (if (and (boundp 'my/ediff-revision-session-p)
                    my/ediff-revision-session-p)
                (ediff-dispose-of-variant-according-to-user
            ...
            ;; CC Note: Reset global state variable `my/ediff-revision-session-p'.
            (if (and (boundp 'my/ediff-revision-session-p)
                    my/ediff-revision-session-p)
                (setq my/ediff-revision-session-p nil))))

    (defun my/stash-window-configuration-for-ediff ()
        "Store window configuration to register 🧊.
Use of emoji is to avoid potential use of keyboard character to reference
the register."
        (window-configuration-to-register ?🧊))

    (defun my/restore-window-configuration-for-ediff ()
...
the register."
    ...
    (add-hook 'ediff-before-setup-hook #'my/stash-window-configuration-for-ediff)
    (add-hook 'ediff-after-quit-hook-internal #'my/restore-window-configuration-for-ediff))
