...
static libraries. Built with `-std=c++11 -Wall -Wextra -Werror`.

## Layout

- `cmd/<binary>/`: binaries, CLIs, and tools. One subdir per binary (e.g.
  `typhoond/`, `nntpcli/`, `tools/`).
- `lib/<module>/`: C++ libraries. One subdir per module; each is on the include
  path and statically linked.
- `top/`: example configs (`*-EXAMPLE`), `CHANGES.<binary>` release notes,
  protocol/error files, etc.
...
changed, I'll handle staging, commits and push.
...
- The repo has in-tree predecessors to many STL types (`lib/auto_ptr`,
  ...
  - **Exceptions:** `string` (in `lib/string/`, ref-counted, *not*
    ...
    `std::string_view`). These are pervasive across the codebase and the
    ref-counting/lifetime semantics matter — keep using them, don't swap to
